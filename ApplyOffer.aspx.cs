        ...
        protected void apply_offer(object sender, EventArgs e)
        {
           // try
            //{
                ...
                cmd.Parameters.Add(new SqlParameter("@serial", decimal.Parse(serial)));


                //Save the output from the procedure
                SqlParameter check = cmd.Parameters.Add("@output", SqlDbType.Int);
                check.Direction = ParameterDirection.Output;
                SqlParameter checkp = cmd.Parameters.Add("@outproduct", SqlDbType.Int);
                checkp.Direction = ParameterDirection.Output;
                SqlParameter checko = cmd.Parameters.Add("@outoffer", SqlDbType.Int);
                checko.Direction = ParameterDirection.Output;
                ...
                if (checko.Value.ToString().Equals("1") & checkp.Value.ToString().Equals("1"))
                {
                ...
                {
                    if (checko.Value.ToString().Equals("0"))
                    {
                        Response.Write("Offer doesnot exist");
                    }
                    else
                    {
                        if (checkp.Value.ToString().Equals("0"))
                        {
                            Response.Write("Product DoesNot Exist");
                        }
                    }
                }
            //}
